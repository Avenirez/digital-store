...
// GetNextSeq returns the next sequence number in 000-999 range.
func (r *OrderRepo) GetNextSeq(ctx context.Context) (int, error) {
	query := `
		SELECT COALESCE(MAX(
			CASE 
	...
	`
	var nextSeq int
	err := r.db.QueryRow(ctx, query).Scan(&nextSeq)
	if err != nil {
		return 0, err
	}
	return nextSeq % 1000, nil
}
